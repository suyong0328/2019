...
public class Sub_boardDao {
	...
	public int insertSubboard(Sub_board sub_board) {
		...
		int number = 0;
		String sql1 = "select nvl(max(sub_num),0)+1 from pj_sub_freeboard";// 글번호
		String sql2 = "insert into pj_sub_freeboard(sub_num,sub_writer,sub_content,sub_password,ref,reg_date) "
	...
	public List<Sub_board> getSubboard(int num) {
		...
		String sql = "select * from pj_sub_freeboard where ref=? order by sub_num";
		try {
			conn = getConnection();
	...
	public Sub_board getsubBoard(int subnum) {
		...
		ResultSet rs = null;
		String sql = "select * from pj_sub_freeboard where sub_num=?";
		Sub_board sub_board = new Sub_board();
		try {
			conn = getConnection();
			pstmt = conn.prepareStatement(sql);
	...
	public int useCheck(int num, String password) {
		...
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		String sql = "select sub_password from pj_sub_freeboard where sub_num=?";
		try {
	...
	public int deletesubBoard(int num) {
		...
		PreparedStatement pstmt = null;
		String sql = "delete from pj_sub_freeboard where sub_num =?";
		try {
			  pstmt = conn.prepareStatement(sql);
			  pstmt.setInt(1, num);
			  result = pstmt.executeUpdate();
	...
	public int getTotal(int num) {
		int total = 0;
		String sql = "select count(*) from pj_sub_freeboard where ref=?";// 글번호
		try {
			conn = getConnection();
			pstmt = conn.prepareStatement(sql);
			pstmt.setInt(1, num);
	...
	public List<Sub_board> selectList(int startRow, int endRow,int num) {
		...
		String sql = "select * from (select rownum rn, a.* from "
				+ "  (select * from pj_sub_freeboard where ref=? order by sub_num desc)a)  where rn between ? and ?";
		try {
			conn = getConnection();
			pstmt = conn.prepareStatement(sql);
			pstmt.setInt(1, num);
